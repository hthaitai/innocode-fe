...
function MentorAppeal() {
  ...
  // Fetch mentor's team
  const { data: myTeamsData, isLoading: myTeamLoading } = useGetMyTeamQuery(
    undefined,
    {
      skip: !user?.id,
    }
  ...
  const myTeam = useMemo(() => {
    if (!myTeamsData || !selectedContestId || !Array.isArray(myTeamsData))
      return null
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-sm text-[#7A7574]">
                        {studentRounds.length}{" "}
                        {studentRounds.length !== 1
                          ? t("appeal.roundsUnit")
                          : t("appeal.roundUnit")}
                      </span>
                    </div>
                    ...
                    {studentRounds.map((round, idx) => (
                      ...
                      >
                        <div className="flex items-start justify-between">
                          ...
                          >
                            <Icon icon="mdi:bullhorn-outline" width={16} />
                  ...
                  <div className="bg-red-50 border border-red-200 rounded-[8px] p-4 text-center">
                    ...
                    <div className="text-sm text-red-500 space-y-1">
                      ...
                      {appealsError?.statusCode && (
                        <p className="text-xs">
                          Status Code: {appealsError.statusCode}
                        </p>
                      )}
                    </div>
                              ...
                              <div className="flex items-center gap-3 mb-3">
                                <div className="px-3 py-1 bg-[#f9fafb] rounded-[5px] border border-[#E5E5E5]">
                                  <span className="text-xs font-medium text-[#7A7574]">
                                    {t("appeal.appealNumber")}
                                    {appealId?.substring(0, 8) || "N/A"}
                                  </span>
                                </div>
