    ...
    const fetchContest = async () => {
      try {
        ...
        while (skippedPages < maxSkipPages) {
          ...
          if (nonDraftContests.length > 0 || !paginationData?.hasNextPage || currentPage >= (paginationData?.totalPages || 1)) {
            if (paginationData) {
            ...
            if (skippedPages > 0) {
              console.log(`⏭️ Auto-skipped ${skippedPages} page(s) with only drafts`);
            }
          ...
          if (paginationData?.hasNextPage && currentPage < paginationData.totalPages) {
            currentPage++;
            skippedPages++;
            setIsAutoSkipping(true);
            console.log(`⏭️ Skipping page ${currentPage - 1} (all drafts), fetching page ${currentPage}`);
          } else {
