...
const useStudentAutoEvaluation = (contestId, roundId, problem = null) => {
  ...
  const handleClearCode = () => {
    openModal("confirm", {
      title: "Clear Code",
      description:
        "Are you sure you want to clear your code? This action cannot be undone.",
      onConfirm: () => {
        setCode("")
        setSubmitResponseCases(null)
  ...
  const handleRunCode = async () => {
    ...
    try {
      ...
      const newSubmissionId = result?.data?.submissionId || result?.submissionId
      ...
      const mergedDetails = testResult.details.map((detail) => {
        ...
        if (matchingCase) {
          return {
            ...
            actual:
              matchingCase.actual !== undefined
                ? matchingCase.actual
                : detail.actual,
            expected:
              matchingCase.expected !== undefined
                ? matchingCase.expected
                : detail.expected,
          }
        }
        ...
        if (submitResponseCases[index]) {
          return {
            ...detail,
            actual:
              submitResponseCases[index].actual !== undefined
                ? submitResponseCases[index].actual
                : detail.actual,
            expected:
              submitResponseCases[index].expected !== undefined
