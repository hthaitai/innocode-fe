...
const AutoTestResult = () => {
  ...
  const {
    ...
    error: contestError,
  } = useContestDetail(shouldSkipContest ? null : contestId)
  const {
    data: testResultData,
        ...
        <div className="max-w-5xl mt-[38px] mx-auto">
          <div className="bg-white border border-[#E5E5E5] rounded-[8px] p-6 text-center">
            ...
            <p className="text-[#7A7574]">
              {t("autoTest.failedToLoadContest")}
            </p>
            <button
              onClick={() => navigate("/contests")}
              className="button-orange mt-4"
            >
              {t("autoTest.backToContests")}
            </button>
          </div>
        </div>
      </PageContainer>
      ...
      <PageContainer bg={false}>
        <div className="flex justify-center items-center h-[400px]">
          <div className="text-center">
            ...
            />
            <p className="text-[#7A7574]">{t("autoTest.loadingResults")}</p>
          </div>
        </div>
      </PageContainer>
    )
      ...
      <PageContainer bg={false}>
        <div className="max-w-5xl mt-[38px] mx-auto">
          <div className="bg-white border border-[#E5E5E5] rounded-[8px] p-6 text-center">
            ...
            />
            <p className="text-[#7A7574]">
              {t("autoTest.failedToLoadResults")}
            </p>
            <p className="text-xs text-red-500 mt-2">
              {resultError?.data?.message ||
                resultError?.message ||
                t("autoTest.unknownError")}
            </p>
            <button
              onClick={() => navigate(`/contest-detail/${contestId}`)}
              className="button-orange mt-4"
            >
              {t("autoTest.backToContest")}
            </button>
          </div>
        </div>
      </PageContainer>
      ...
      <PageContainer bg={false}>
        <div className="max-w-5xl mt-[38px] mx-auto">
          <div className="bg-white border border-[#E5E5E5] rounded-[8px] p-6 text-center">
            ...
            />
            <p className="text-[#7A7574]">{t("autoTest.noResultsFound")}</p>
            <button
              onClick={() => navigate(`/contest-detail/${contestId}`)}
              className="button-orange mt-4"
            >
  ...
  // Safely get counts
  const passedCount =
    testResult?.details?.filter((d) => d.note === "success").length || 0
    ...
    <PageContainer bg={false}>
      <div className="max-w-6xl mt-[38px] mx-auto">
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-[8px] p-6 mb-4">
          ...
          >
            <ArrowLeft size={16} />
            <span className="text-sm">{t("autoTest.backToContest")}</span>
          </button>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-[#2d3748]">
                {round?.roundName || t("autoTest.title")}
              </h1>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-medium text-sm">
                  {t("autoTest.status")}: {testResult?.status}
                </span>
              </div>
            </div>
          )}
        ...
        <div className="bg-white border border-[#E5E5E5] rounded-[8px] p-6">
          <h2 className="text-lg font-semibold text-[#2d3748] mb-4">
            {t("autoTest.submissionInfo")}
          </h2>
          <div className="space-y-2 text-sm">
            {testResult?.teamName && (
              <p className="text-[#4a5568]">
                <span className="font-medium">{t("autoTest.team")}:</span>{" "}
                {testResult.teamName}
              </p>
            )}
