...
import CalendarPicker from "./datetimefieldfluent/CalendarPicker"
import { EASING } from "./ui/easing"

const DateFieldFluent = ({
  label,
  name,
  value,
  onChange,
  ...
  const handleSave = () => {
    if (selectedDate) {
      onChange({
        target: { name, value: selectedDate.format("YYYY-MM-DD") },
      })
      setInitialDate(selectedDate)
    }
    setShowPicker(false)
  ...
  return (
    <div className="flex flex-col w-full relative" ref={containerRef}>
      ...
      <div className="relative inline-block w-full">
        <div
          className={`inline-flex items-center justify-between gap-3 rounded-[5px] bg-white transition-all duration-200 px-3 min-h-[32px] w-full ${
            error
        ...
        >
          <span className="text-sm text-gray-700">
            {displayDate ? displayDate.format("DD/MM/YYYY") : "dd/mm/yyyy"}
          </span>
          <ChevronDown
            size={16}
        ...
        <AnimatePresence>
          {showPicker && (
            <motion.div
              ...
              className="w-max absolute top-full bg-white border border-[#E5E5E5] rounded-[5px] shadow-lg p-5 z-[9999]"
              style={{
                ...pickerPosition,
              }}
              ref={pickerRef}
...
export default DateFieldFluent
